...
public class RecordExtraction : IRecordExtraction
{
    public IEnumerable<ParentAndSubsidiaries> ExtractParentsAndSubsidiaries(IEnumerable<CompaniesHouseCompany> source)
    {
        ...
        foreach (var group in groups)
        {
            ...
            if (parent == null)
            {
                parent = new CompaniesHouseCompany() { organisation_id = group.Key, organisation_name = "orphan", parent_child = "child", joiner_date = string.Empty, reporting_type = string.Empty };
            }
