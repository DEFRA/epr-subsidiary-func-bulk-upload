...
public class CompaniesHouseCompanyMap : ClassMap<CompaniesHouseCompany>
{
    ...
    private static List<UploadFileErrorModel> GetRowValidationErrors(IReaderRow row)
    {
        ...
        if (string.IsNullOrEmpty(row.GetField(nameof(CompaniesHouseCompany.companies_house_number)))
            && string.IsNullOrEmpty(row.GetField(nameof(CompaniesHouseCompany.franchisee_licensee_tenant))))
        {
            errors.Add(
                CreateError(
        ...
        if (!string.IsNullOrEmpty(row.GetField(nameof(CompaniesHouseCompany.companies_house_number)))
            && row.GetField(nameof(CompaniesHouseCompany.companies_house_number)).Length > 8)
        {
            errors.Add(
                CreateError(
                    lineNumber, rawData, BulkUpdateErrors.InvalidFieldLengthErrorMessage, BulkUpdateErrors.InvalidFieldLengthError));
        }
